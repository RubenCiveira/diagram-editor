 ...
 */
export function useNestValidation(nodes: any[], palette: PaletteInterface | undefined) {
  return useCallback(
    (sourceNodeId: string, targetNodeId: string) => {
      ...
      const sOut = srcType.acceptsChilds?.() ?? true;
      const tIn = tgtType.acceptsParents?.() ?? true;

      if (!sOut) return { ok: false, reason: 'El origen no admite hijos' };
      if (!tIn) return { ok: false, reason: 'El destino no admite padres' };

      try {
        srcType.verifyNestChild?.({
          source: src as any,
          target: tgt,
          targetKind: tgt.kind,
          targetType: tgtType,
        });
        tgtType.verifyNestIntoParent?.({
          source: src as any,
          target: tgt,
          sourceKind: src.kind,
          sourceType: srcType,
